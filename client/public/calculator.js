...
function initCalculatorForSubject() {
    ...
    // Check if any selected subjects need calculator (for exam mode via URL)
    if (subjects) {
        const selectedSubjects = subjects.split(',');
        needsCalculator = selectedSubjects.some(s => calculatorSubjects.includes(s.toLowerCase()));
    }

    // Check exam subjects from session storage (React exam room flow)
    if (!needsCalculator) {
        try {
            const examSubjectsRaw = sessionStorage.getItem('examSubjects');
            if (examSubjectsRaw) {
                const selectedSubjects = JSON.parse(examSubjectsRaw);
                if (Array.isArray(selectedSubjects)) {
                    needsCalculator = selectedSubjects.some(s => calculatorSubjects.includes(String(s).toLowerCase()));
                }
            }
        } catch (_) {
            // Ignore parse errors and keep current needsCalculator value
        }
    }
